...
namespace ReminderService.Repository
{
    ...
    public class ReminderRepository : IReminderRepository
    {
        ...
        public Task CreateReminder(string userId, string email, ReminderSchedule schedule)
        {
            ...
            var maxNewsId = (list == null || list.NewsReminders == null || list.NewsReminders.Count == 0) ? schedule.NewsId : list.NewsReminders.Max(x => x.NewsId);
            ...
            else
            {
                ...
                var update = Builders<Reminder>.Update.Push(x => x.NewsReminders, schedule);
                var filter = Builders<Reminder>.Filter.And(
                Builders<Reminder>.Filter.Where(x => x.UserId == userId));
                var filterOutput = _reminderContext.Reminders.Find(x => x.UserId == userId).ToList();
                _reminderContext.Reminders.FindOneAndUpdate(filter, update);
            }
            var createdReminder = _reminderContext.Reminders.Find(x => x.UserId == userId).ToList();
